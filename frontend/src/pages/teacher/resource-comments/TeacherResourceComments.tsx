...
const TeacherResourceComments: React.FC = () => {
  ...
  const resourceQuery = useApi(
    () =>
      resourceId
        ? teacherService.getResourceDetails(resourceId)
  ...
  const commentsQuery = useApi(
    () =>
      resourceId
        ? teacherService.getResourceComments(resourceId, {
            page: currentPage,
            limit: pageSize,
          })
        : Promise.reject("No resource ID"),
  ...
  const createCommentMutation = useMutation(
    ({ resourceId, content }: { resourceId: string; content: string }) =>
      teacherService.createResourceComment(resourceId, { content }),
    {
      onSuccess: () => {
  ...
  const updateCommentMutation = useMutation(
    ({ commentId, content }: { commentId: string; content: string }) =>
      teacherService.updateResourceComment(commentId, { content }),
    {
      onSuccess: () => {
        notification.success({
          message: "Success",
  ...
  const deleteCommentMutation = useMutation(
    teacherService.deleteResourceComment,
    {
      onSuccess: () => {
        notification.success({
          message: "Success",
  ...
  const isCommentOwner = (comment: Comment) => {
    return comment.teacher?.id === user?.id;
  };
...
export default TeacherResourceComments;
